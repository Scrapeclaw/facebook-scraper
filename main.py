#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Facebook Page & Group Scraper - Main Entry Point
ClawHub Skill for OpenClaw Agent Integration

Commands:
    discover  - Discover Facebook pages/groups via Google Search API
    scrape    - Scrape Facebook pages/groups using browser automation
    list      - List available queue files
    export    - Export scraped data to JSON/CSV
"""

import sys
import json
import asyncio
import argparse
from pathlib import Path


def main():
    parser = argparse.ArgumentParser(
        prog='facebook-scraper',
        description='Facebook Page & Group Discovery and Scraping Tool',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s discover --location "New York" --category "restaurant"
  %(prog)s discover --location "Miami" --category "fitness" --type group
  %(prog)s scrape data/queue/NewYork_restaurant_page_20260220.json
  %(prog)s scrape --page-name examplebusiness
  %(prog)s scrape --page-name examplegroup --type group
  %(prog)s list
  %(prog)s export --format json
        """
    )
    
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # Discover command
    discover_parser = subparsers.add_parser('discover', help='Discover Facebook pages/groups via Google API')
    discover_parser.add_argument('--location', '-l', type=str, help='Location/city to search')
    discover_parser.add_argument('--category', '-c', type=str, help='Category to search')
    discover_parser.add_argument('--type', '-t', type=str, choices=['page', 'group', 'profile'], default='page', help='Entity type to discover')
    discover_parser.add_argument('--count', '-n', type=int, default=10, help='Number of pages/groups to discover')
    discover_parser.add_argument('--batch', '-b', action='store_true', help='Batch mode for multiple locations')
    discover_parser.add_argument('--output', '-o', type=str, choices=['json', 'text'], default='text')
    
    # Scrape command
    scrape_parser = subparsers.add_parser('scrape', help='Scrape Facebook pages/groups')
    scrape_parser.add_argument('queue_file', nargs='?', help='Queue file to scrape')
    scrape_parser.add_argument('--page-name', '-p', type=str, help='Single page/group name to scrape')
    scrape_parser.add_argument('--type', '-t', type=str, choices=['page', 'group', 'profile'], default='page', help='Entity type')
    scrape_parser.add_argument('--resume', '-r', action='store_true', default=True, help='Resume from checkpoint')
    scrape_parser.add_argument('--headless', action='store_true', help='Run browser in headless mode')
    scrape_parser.add_argument('--output', '-o', type=str, choices=['json', 'text'], default='text')
    
    # List command
    list_parser = subparsers.add_parser('list', help='List available queue files')
    
    # Export command
    export_parser = subparsers.add_parser('export', help='Export scraped data')
    export_parser.add_argument('--format', '-f', type=str, choices=['json', 'csv', 'both'], default='both')
    
    args = parser.parse_args()
    
    if args.command == 'discover':
        from discovery import discover_command, interactive_discovery, batch_discovery
        
        if args.batch:
            batch_discovery()
        elif args.location and args.category:
            result = discover_command(
                args.location, 
                args.category, 
                args.type,
                args.count, 
                args.output == 'json'
            )
            if args.output == 'json' and result:
                print(json.dumps(result, indent=2))
        else:
            interactive_discovery()
    
    elif args.command == 'scrape':
        from scraper import scrape_from_queue, scrape_single, list_queue_files
        
        if args.page_name:
            result = asyncio.run(scrape_single(args.page_name, args.type, args.output == 'json'))
            if args.output == 'json' and result:
                print(json.dumps(result, indent=2))
        elif args.queue_file:
            asyncio.run(scrape_from_queue(args.queue_file, args.resume))
        else:
            # Interactive queue selection
            list_queue_files()
            print("\nUsage: python main.py scrape <queue_file>")
    
    elif args.command == 'list':
        from scraper import list_queue_files
        list_queue_files()
    
    elif args.command == 'export':
        from scraper import export_data
        export_data(args.format)
    
    else:
        parser.print_help()


if __name__ == '__main__':
    main()
